...
public class PolyclinicTests
{
    ...
    public async Task GetPatientsByDoctor_ReturnsCorrectPatients(int doctorId)
    {
        ...
        NewMethod(result, expectedPatients);
    ...
    private static void NewMethod(IList<Model.Patient> result, List<Model.Patient> expectedPatients)
    {
    ...
    /// </summary>
    [Fact]
    public async Task GetHealthyPatientsReturnsCorrectPatients()
    {
        // Act
        ...
        // Assert
        Assert.NotNull(result);
        Assert.Equal(2, result.Count); // ќжидаем 2 здоровых пациента

        var healthyPatientIds = new[] { 1, 3 }; // ID пациентов со статусом "«доров"
        foreach (var id in healthyPatientIds)
        {
            Assert.Contains(result, p => p.Id == id);
        }
    }
